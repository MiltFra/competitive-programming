...
parser = argparse.ArgumentParser(
    description='Testing tool for the Dragon Balls problem')
...
if args.inputfile is not None:
    ...
    import random
    pts = [(random.randint(0, 10**6), random.randint(0, 10**6))]
    print(pts)
    points = len(pts)
with subprocess.Popen(" ".join(args.program),
    ...
    try:
        ...
        while True:
            ...
            if res == 0:
                ...
                if len(pts) == 0:
                    ...
                    assert p.wait(
                    ) == 0, 'Did not exit cleanly after finishing'
                    break
            if steps == 1000:
        ...
        sys.stderr.flush()
        sys.stdout.write(
            'Guessed {} times, {} points remaining, exit code: {}\n'.format(
                steps, len(pts), p.wait()))
        sys.stdout.flush()
